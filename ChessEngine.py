...
class GameState():
    ...
    def getValidMoves(self):
        ...
        for i in range(len(moves) - 1, -1, -1): # when removing from the list, go backwords :)
            self.makeMove(moves[i])     
            # 3. generate all opponent's moves
            # 4. for each of those moves, check if they attack your king
            self.whiteToMove = not self.whiteToMove # we need this as the makeMove() did swap the players once
            if self.inCheck():
                # 5. if they do, it's not a valid move
                moves.remove(moves[i])
            self.whiteToMove = not self.whiteToMove # we need this to return every thing as before
            self.undoMove()
    ...
    def getKingMove(self, r, c, moves):
        kingMoves = ((-1, 0), (0, -1), (1, 0), (0, 1),  # from the rock
                     (-1, -1), (-1, 1), (1, -1), (1, 1))  # from the bishop
        allyColor = "w" if self.whiteToMove else "b"
        for i in range(8):
